...
GraphicsRenderPassNode::GraphicsRenderPassNode(
    const std::shared_ptr<VkGPUContext> &gpuCtx, const std::string &name,
    const std::vector<VkAttachmentDescription> &attachments,
    const std::vector<VkSubpassDependency> &dependencies,
    const std::vector<VkSubpassDescription> &subPasses, const float width,
    const float height, const std::vector<VkClearValue> &clearValues) {
  this->gpuCtx = gpuCtx;
  this->name = name;
  this->width = width;
  this->height = height;
...
VkResult GraphicsRenderPassNode::CreateComputeGraphNode() {
  ...
  this->renderPass = std::make_shared<VkGPURenderPass>(
      gpuCtx, attachments, subPassDependencies, subPasses, renderArea,
      clearValues);
...
void GraphicsRenderPassNode::Compute(const VkCommandBuffer commandBuffer) {
  ...
  const std::vector<VkRect2D> scissors = {{
      .offset = {0, 0},
      .extent = {static_cast<uint32_t>(this->width),
...
void GraphicsRenderPassNode::Destroy() { this->renderPass->Destroy(); }
