...
VkResult SubComputeGraph::Init() {
  VkResult ret =
      VkGPUHelper::CreateFence(gpuCtx->GetCurrentDevice(), &this->computeFence);
  if (ret != VK_SUCCESS) {
    Logger() << "Failed to create compute fence, err =" << string_VkResult(ret)
             << std::endl;
    return ret;
  }

  ret = VkGPUHelper::CreateSemaphore(gpuCtx->GetCurrentDevice(),
                                     &this->computeDoneSemaphore);
  if (ret != VK_SUCCESS) {
    Logger() << "Failed to create compute done semaphore, err ="
             << string_VkResult(ret) << std::endl;
    return ret;
  }

  queue = this->gpuCtx->DispatchQueue(
      VK_QUEUE_COMPUTE_BIT | VK_QUEUE_GRAPHICS_BIT | VK_QUEUE_TRANSFER_BIT);
  if (queue.queue == VK_NULL_HANDLE) {
    Logger() << "Failed to create compute queue, err =" << string_VkResult(ret)
             << std::endl;
    return ret;
  }

  ret = VkGPUHelper::AllocateCommandBuffers(
      gpuCtx->GetCurrentDevice(),
      gpuCtx->GetCommandPool(queue.queueFamilyIndex), 1, &this->commandBuffer);
  if (ret != VK_SUCCESS) {
...
void SubComputeGraph::AddDependence(
    const std::shared_ptr<SubComputeGraph> &dependence) {
  this->dependence.push_back(dependence);
}

void SubComputeGraph::AddComputeGraphNode(
...
VkResult SubComputeGraph::Compute() const {
  ...
  const VkPipelineStageFlags submitWaitDstStageMask =
      VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT;

  std::vector<VkSubmitInfo> submitInfos;
  submitInfos.push_back(VkGPUHelper::BuildSubmitInfo(
      &submitWaitDstStageMask, submitCommandBuffers, submitSignalSemaphores,
      submitWaitSemaphores));

  const VkResult ret =
      VkGPUHelper::GPUQueueSubmit(queue.queue, submitInfos, this->computeFence);
  if (ret != VK_SUCCESS) {
    Logger() << "Failed to submit command buffer, err =" << string_VkResult(ret)
             << std::endl;
    return ret;
  }
  return ret;
...
void SubComputeGraph::Destroy() {
  ...
  if (computeDoneSemaphore != VK_NULL_HANDLE) {
    vkDestroySemaphore(gpuCtx->GetCurrentDevice(), computeDoneSemaphore,
                       nullptr);
  ...
  if (commandBuffer != VK_NULL_HANDLE) {
    ...
    vkFreeCommandBuffers(gpuCtx->GetCurrentDevice(),
                         ...
                         freeCommandBuffers.size(), freeCommandBuffers.data());
