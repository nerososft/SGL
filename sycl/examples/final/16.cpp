...
int main()
{
  ...
  float *matB = (float *)malloc(N * M * sizeof(float));

  for(float index = 0.0f; index < N * M; ++index) {
    matA[(unsigned int)index] = index;
    matB[(unsigned int)index] = index + 0.5f;
  }
  float *matC = (float *)malloc(N * M * sizeof(float));

  uint64_t startMs = GetCurrentMonoMs();
  sycl::queue q;
  {
