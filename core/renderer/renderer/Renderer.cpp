...
bool Renderer::Init(const std::vector<const char *> &requiredExtensions,
  ...
  if (this->renderMode == RENDER_MODE_ONSCREEN) {
    ...
    result = this->swapChain->CreateSwapChain(
        GetSurface(this->gpuCtx->GetInstance()), this->width, this->height,
        queueFamilies);
    if (result != VK_SUCCESS) {
      Logger() << Logger::ERROR << "Failed to create swap chain!" << std::endl;
      return false;
  ...
  if (!this->subComputeGraph) {
    Logger() << Logger::ERROR << "Failed to create sub compute graph!"
             << std::endl;
    return false;
  }
  result = this->subComputeGraph->Init();
  if (result != VK_SUCCESS) {
    Logger() << Logger::ERROR << "Failed to initialize sub compute graph!"
             << std::endl;
    return false;
  }
  ...
  mainRenderPassNode = std::make_shared<GraphicsRenderPassNode>(
      ...
      this->width, this->height, clearValues);
  if (mainRenderPassNode == nullptr) {
    Logger() << Logger::ERROR << "Failed to create graphics render pass node!"
             << std::endl;
    return false;
  }
  ...
  if (result != VK_SUCCESS) {
    Logger() << Logger::ERROR
             << "Failed to create render pass compute graph node!" << std::endl;
    return false;
  }

  this->framebuffer = std::make_shared<VkGPUFramebuffer>(
      this->gpuCtx, this->width, this->height,
  ...
  result = offScreenBuffer->AllocateAndBind(GPU_BUFFER_TYPE_STORAGE_SHARED,
                                            this->width * this->height * 4);
  if (result != VK_SUCCESS) {
    Logger() << Logger::ERROR << "Failed to allocate offscreen buffer!"
             << std::endl;
    return false;
  }
  ...
  offScreenCopyNode = std::make_shared<ImageToBufferCopyNode>(
      this->gpuCtx, "offScreenCopy", srcCopyInfo, dstCopyInfo);
  if (offScreenCopyNode == nullptr) {
    Logger() << Logger::ERROR << "Failed to create offscreen copy node!"
             << std::endl;
    return false;
  }
  ...
  if (renderMode == RENDER_MODE_ONSCREEN) {
    result = VkGPUHelper::CreateFence(this->gpuCtx->GetCurrentDevice(),
                                      &this->renderFinishedFence);
    if (result != VK_SUCCESS) {
      Logger() << Logger::ERROR << "Failed to create fence!" << std::endl;
      return false;
    }
    result = VkGPUHelper::CreateSemaphore(this->gpuCtx->GetCurrentDevice(),
    ...
    result = VkGPUHelper::AllocateCommandBuffers(
        this->gpuCtx->GetCurrentDevice(), this->gpuCtx->GetCommandPool(0), 1,
        &presentCmdBuffer);
    if (result != VK_SUCCESS) {
      Logger() << Logger::ERROR << "Failed to allocate command buffers!"
               << std::endl;
      return false;
    }
  }
...
VkResult Renderer::Present() const {
  ...
  const VkResult result = vkAcquireNextImageKHR(
      this->gpuCtx->GetCurrentDevice(), this->swapChain->GetSwapChain(),
      UINT64_MAX, this->imageAvailableSemaphore, this->renderFinishedFence,
      &imageIndex);
  if ((result != VK_SUCCESS) && (result != VK_SUBOPTIMAL_KHR)) {
    Logger() << Logger::ERROR << "Failed to acquire swap chain image, err="
  ...
  imageMemoryBarriers.push_back(VkGPUHelper::BuildImageMemoryBarrier(
  ...
  vkCmdPipelineBarrier(
      this->presentCmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
      VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr,
      imageMemoryBarriers.size(), imageMemoryBarriers.data());
  vkCmdCopyImage(this->presentCmdBuffer, this->framebuffer->GetColorImage(),
                 VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                 this->swapChain->GetSwapChainImg(imageIndex),
  ...
  for (VkSemaphore waitSemaphore :
       this->computeGraph->GetComputeDoneSemaphores()) {
  ...
  VkGPUHelper::GPUQueueSubmit(this->gpuCtx
                                  ->DispatchQueue(VK_QUEUE_TRANSFER_BIT |
                                                  VK_QUEUE_COMPUTE_BIT |
                                                  VK_QUEUE_GRAPHICS_BIT)
                                  .queue,
                              submitInfos, renderFinishedFence);

  vkWaitForFences(this->gpuCtx->GetCurrentDevice(), fences.size(),
  ...
  vkQueuePresentKHR(this->gpuCtx->DispatchQueue(VK_QUEUE_GRAPHICS_BIT).queue,
                    &presentInfo);
  return VK_SUCCESS;
}
