...
VkResult
pathBlurFilter::Apply(const std::shared_ptr<VkGPUContext> &gpuCtx,
  ...
  this->pathblurFilterParams.imageSize.bytesPerLine =
      this->pathblurFilterParams.imageSize.width * 4;
  ...
  startposBuffer->AllocateAndBind(GPU_BUFFER_TYPE_UNIFORM,
  ...
  endposBuffer->AllocateAndBind(GPU_BUFFER_TYPE_UNIFORM,
                                vecsize * sizeof(float));
  ...
  startvecBuffer->AllocateAndBind(GPU_BUFFER_TYPE_UNIFORM,
                                  vecsize * sizeof(float));
  ...
  endvecBuffer->AllocateAndBind(GPU_BUFFER_TYPE_UNIFORM,
                                vecsize * sizeof(float));
  ...
  vecBuffer = std::make_shared<VkGPUBuffer>(gpuCtx);
  vecBuffer->AllocateAndBind(GPU_BUFFER_TYPE_STORAGE_LOCAL,
                             k_size * sizeof(float));
  ...
  const auto vecCalculateNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "vecpathblurCalculate", SHADER(vecpathblur.comp.glsl.spv),
  ...
  vecCalculateNode->AddComputeElement(
      {.pushConstantInfo = pushConstantInfo, .buffers = vecPipelineBuffers});
  ...
  const auto kCalculateNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "pathblur", SHADER(pathblur.comp.glsl.spv), pushConstantInfo.size,
      descriptorSetLayoutBindings2, (inputImageInfo[0].width + 31) / 32,
