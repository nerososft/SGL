...
VkResult ColorhalftoneFilter::Apply(
  ...
  this->colorhalftoneFilterParams.imageSize.bytesPerLine =
      this->colorhalftoneFilterParams.imageSize.width * 4;
  ...
  pipelineNodelookupInput.buf.buffer = lookupBuffer->GetBuffer();
  pipelineNodelookupInput.buf.bufferSize = size * sizeof(float);
  ...
  const auto kCalculateNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "colorhalftone", SHADER(colorhalftone.comp.glsl.spv),
