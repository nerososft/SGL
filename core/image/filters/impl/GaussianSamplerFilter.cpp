...
VkResult GaussianSamplerFilter::Apply(
  ...
  const auto guassianSamplerNode = std::make_shared<ComputePipelineNode>(
  ...
  ret = guassianSamplerNode->CreateComputeGraphNode();
  if (ret != VK_SUCCESS) {
    Logger() << "Failed to create compute graph, err =" << string_VkResult(ret)
             << std::endl;
    return ret;
  ...
  guassianSamplerNode->AddComputeElement(
      {.pushConstantInfo = pushConstantInfo, .buffers = vPipelineBuffers});

  computeSubGraph->AddComputeGraphNode(guassianSamplerNode);
