...
VkResult
RotationBlurFilter::Apply(const std::shared_ptr<VkGPUContext> &gpuCtx,
  ...
  this->rotationblurFilterParams.imageSize.bytesPerLine =
      this->rotationblurFilterParams.imageSize.width * 4;
  ...
  const auto kCalculateNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "rotationblur", SHADER(rotationblur.comp.glsl.spv),
