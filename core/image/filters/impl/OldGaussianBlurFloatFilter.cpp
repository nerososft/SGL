...
VkResult OldGaussianBlurFloatFilter::Apply(
  ...
  const auto gaussianVerticalNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "OldGaussianVerticalBlurFloat",
      SHADER(vertical_blur_old_float.comp.glsl.spv), pushConstantInfo.size,
  ...
  const auto gaussianHorizontalNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "OldGaussianHorizontalBlurFloat",
      SHADER(horizontal_blur_old_float.comp.glsl.spv), pushConstantInfo.size,
      descriptorSetLayoutBindings, (inputImageInfo[0].width + 31) / 32,
      (inputImageInfo[0].height + 31) / 32, 1);
