...
VkResult
BlackWhiteFilter::Apply(const std::shared_ptr<VkGPUContext> &gpuCtx,
  ...
  this->wFilterParams.imageSize.bytesPerLine =
      this->wFilterParams.imageSize.width * 4;
  ...
  const auto kCalculateNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "BlackWhite", SHADER(black_white.comp.glsl.spv),
