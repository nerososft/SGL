...
VkDescriptorSetLayoutBinding VkGPUHelper::BuildDescriptorSetLayoutBinding(
    const uint32_t binding, const VkDescriptorType descriptorType,
...
VkWriteDescriptorSet VkGPUHelper::BuildWriteSamplerDescriptorSet(
    const VkDescriptorSet descriptorSet, const uint32_t dstBinding,
    const VkDescriptorImageInfo *imageInfo) {
  return BuildWriteDescriptorSet(descriptorSet, dstBinding,
                                 VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                 imageInfo, nullptr, nullptr);
}

VkResult VkGPUHelper::CreateSampler(const VkDevice device, VkSampler *sampler) {
  ...
  if (result != VK_SUCCESS) {
    Logger() << "failed to create sampler, err=" << string_VkResult(result)
             << std::endl;
  }
  return result;
}
...
VkResult VkGPUHelper::CreateImageView(const VkDevice device,
  ...
  if (result != VK_SUCCESS) {
    Logger() << "Failed to create image view, err=" << string_VkResult(result)
             << std::endl;
  }
  return result;
}

auto VkGPUHelper::CreateImage(const VkDevice device, const float width,
                              const float height, const VkImageType imageType,
                              const VkFormat format,
                              const VkImageUsageFlags usage,
                              const VkSharingMode sharingMode,
                              const std::vector<uint32_t> &queueFamilies,
                              const VkImageLayout initialLayout, VkImage *image)
    -> VkResult {
  ...
  if (result != VK_SUCCESS) {
    Logger() << "Failed to create image, err=" << string_VkResult(result)
             << std::endl;
  }
  return result;
}

VkResult VkGPUHelper::CreateFramebuffer(
    const VkDevice device, const uint32_t width, const uint32_t height,
    const std::vector<VkImageView> &attachments, const VkRenderPass renderPass,
  ...
  if (ret != VK_SUCCESS) {
    Logger() << "Failed to create framebuffer, err=" << string_VkResult(ret)
             << std::endl;
  }
  return ret;
}
...
VkResult VkGPUHelper::CreateSemaphore(const VkDevice device,
  ...
  if (ret != VK_SUCCESS) {
    Logger() << "failed to create semaphore, err=" << string_VkResult(ret)
             << std::endl;
  }
  return ret;
}
...
VkGPUHelper::GetVkCmdPipelineBarrier2Fn(const VkDevice device) {
  PFN_vkCmdPipelineBarrier2KHR func = nullptr;
  func = reinterpret_cast<PFN_vkCmdPipelineBarrier2KHR>(
      vkGetDeviceProcAddr(device, "vkCmdPipelineBarrier2KHR"));
  return func;
}

void VkGPUHelper::GPUCmdPipelineMemBarrier(
  ...
  GPUCmdPipelineBarrier(commandBuffer, srcStageMask, dstStageMask,
                        dependencyFlags, memoryBarriers, bufferMemoryBarriers,
                        imageMemoryBarrier);
}

VkMemoryBarrier
VkGPUHelper::BuildMemoryBarrier(const VkAccessFlagBits srcAccessMask,
...
void VkGPUHelper::GPUCmdBeginRenderPass(
...
VkResult VkGPUHelper::CreateRenderPass(
  ...
  if (ret != VK_SUCCESS) {
    Logger() << "Failed to create render pass, err=" << string_VkResult(ret)
             << std::endl;
    return ret;
  }
  return ret;
...
VkResult VkGPUHelper::CreateGraphicsPipeline(
    ...
    VkPrimitiveTopology inputAssemblyTopology, VkBool32 primitiveRestartEnable,
  ...
  ret = vkCreateGraphicsPipelines(
      device, pipelineCache, pipelineCreateInfos.size(),
      pipelineCreateInfos.data(), nullptr, pipeline);
  if (ret != VK_SUCCESS) {
...
VkResult VkGPUHelper::CreateUniformBufferAndUploadData(
    const VkDevice device, const std::vector<uint32_t> &queueFamilyIndices,
    const VkPhysicalDeviceMemoryProperties *memoryProperties,
    const VkDeviceSize bufferSize, VkBuffer *buffer,
    VkDeviceMemory *bufferMemory, const void *uploadData) {
  VkResult ret =
      CreateStorageBufferAndBindMem(device, bufferSize, queueFamilyIndices,
                                    memoryProperties, buffer, bufferMemory);
  if (ret != VK_SUCCESS) {
  ...
  if (ret != VK_SUCCESS) {
    Logger() << "Failed to map input storage buffer memory, err="
...
VkResult VkGPUHelper::CreateStorageBufferAndUploadData(
    const VkDevice device, const std::vector<uint32_t> &queueFamilyIndices,
    const VkPhysicalDeviceMemoryProperties *memoryProperties,
    const VkDeviceSize bufferSize, VkBuffer *buffer,
    VkDeviceMemory *bufferMemory, const void *uploadData) {
...
VkSubmitInfo VkGPUHelper::BuildSubmitInfo(
    const VkFlags *submitWaitDstStageMask,
...
VkBufferMemoryBarrier VkGPUHelper::BuildBufferMemoryBarrier(
    const VkAccessFlagBits srcAccessMask, const VkAccessFlagBits dstAccessMask,
    const VkBuffer buffer, const VkDeviceSize size) {
...
VkImageMemoryBarrier VkGPUHelper::BuildImageMemoryBarrier(
    const VkAccessFlagBits srcAccessMask, const VkAccessFlagBits dstAccessMask,
...
void VkGPUHelper::GPUCmdPipelineBufferMemBarrier(
    const VkCommandBuffer commandBuffer,
    const VkPipelineStageFlags srcStageMask,
    const VkPipelineStageFlags dstStageMask,
    const VkDependencyFlags dependencyFlags,
...
void VkGPUHelper::GPUCmdPipelineImageMemBarrier(
    const VkCommandBuffer commandBuffer,
    const VkPipelineStageFlags srcStageMask,
    const VkPipelineStageFlags dstStageMask,
    const VkDependencyFlags dependencyFlags,
...
void VkGPUHelper::GPUCmdPipelineBarrier(
    const VkCommandBuffer commandBuffer,
    const VkPipelineStageFlags srcStageMask,
    const VkPipelineStageFlags dstStageMask,
    const VkDependencyFlags dependencyFlags,
  ...
  vkCmdPipelineBarrier(commandBuffer, srcStageMask, dstStageMask,
                       dependencyFlags, memoryBarriers.size(),
                       memoryBarriers.data(), bufferMemoryBarriers.size(),
                       bufferMemoryBarriers.data(), imageMemoryBarriers.size(),
                       imageMemoryBarriers.data());
}

VkResult
VkGPUHelper::GPUQueueSubmit(const VkQueue queue,
                            const std::vector<VkSubmitInfo> &submitInfos,
                            const VkFence fence) {
  const VkResult ret =
      vkQueueSubmit(queue, submitInfos.size(), submitInfos.data(), fence);
  if (ret != VK_SUCCESS) {
    Logger() << "vkQueueSubmit failed, err=" << string_VkResult(ret)
             << std::endl;
  }
  return ret;
}
...
void VkGPUHelper::GPUCmdPushConstant(
    const VkCommandBuffer commandBuffer, const VkPipelineLayout pipelineLayout,
    const VkShaderStageFlagBits shaderStageFlag, const uint32_t offset,
    const size_t pushConstantSize, const void *pushConstantData) {
  vkCmdPushConstants(commandBuffer, pipelineLayout, shaderStageFlag, offset,
...
VkResult VkGPUHelper::AllocateCommandBuffers(const VkDevice device,
  ...
  if (result != VK_SUCCESS) {
    Logger() << "Failed to allocate command buffer, err = "
...
VkResult VkGPUHelper::CreateFence(const VkDevice device, VkFence *fence) {
  ...
  if (result != VK_SUCCESS) {
    Logger() << "vkCreateFence failed, err=" << string_VkResult(result)
             << std::endl;
  }
  return result;
}

VkWriteDescriptorSet VkGPUHelper::BuildWriteDescriptorSet(
    const VkDescriptorSet descriptorSet, const uint32_t dtsBinding,
    const VkDescriptorType type, const VkDescriptorImageInfo *imageInfo,
...
VkWriteDescriptorSet VkGPUHelper::BuildWriteUniformBufferDescriptorSet(
    const VkDescriptorSet descriptorSet, const uint32_t dstBinding,
...
VkWriteDescriptorSet VkGPUHelper::BuildWriteStorageBufferDescriptorSet(
    const VkDescriptorSet descriptorSet, const uint32_t dstBinding,
    const VkDescriptorBufferInfo *descriptorBufferInfo) {
  return BuildWriteDescriptorSet(descriptorSet, dstBinding,
...
VkResult VkGPUHelper::AllocateDescriptorSets(
    const VkDevice device, const VkDescriptorPool descriptorPool,
  ...
  const VkResult result = vkAllocateDescriptorSets(
      device, &descriptorSetAllocateInfo, descriptorSets);
  if (result != VK_SUCCESS) {
...
VkResult VkGPUHelper::CreateComputePipeline(
    const VkDevice device, const VkPipelineCache pipelineCache,
...
VkResult VkGPUHelper::CreatePipelineLayout(
    const VkDevice device,
    const std::vector<VkDescriptorSetLayout> &descriptorSetLayouts,
  ...
  const VkResult ret = vkCreatePipelineLayout(device, &pipelineLayoutInfo,
                                              nullptr, pipelineLayout);
  if (ret != VK_SUCCESS) {
    Logger() << "failed to create pipeline layout, " << string_VkResult(ret)
             << std::endl;
  }
  return ret;
}

VkResult VkGPUHelper::CreateDescriptorSetLayout(
    const VkDevice device,
  ...
  const VkResult ret = vkCreateDescriptorSetLayout(
...
VkResult VkGPUHelper::CreateBuffer(
  ...
  if (ret != VK_SUCCESS) {
    Logger() << "vkCreateBuffer failed, err=" << string_VkResult(ret)
             << std::endl;
  }
  return ret;
}

uint32_t VkGPUHelper::GetRequiredMemTypeIndex(
    const VkPhysicalDeviceMemoryProperties *memProps,
    const VkMemoryRequirements &memRequirements,
    const uint32_t memoryPropertyFlagsBits) {
  for (uint32_t i = 0; i < memProps->memoryTypeCount; i++) {
    if (memRequirements.memoryTypeBits & (1 << i)) {
      if ((memoryPropertyFlagsBits & memProps->memoryTypes[i].propertyFlags) ==
...
VkResult VkGPUHelper::CreateStorageBufferAndBindMem(
    const VkDevice device, const VkDeviceSize size,
...
VkResult VkGPUHelper::CreateImageAndBindMem(
    const VkDevice device, const float width, const float height,
    const VkImageType imageType, const VkFormat format,
    const VkImageUsageFlags usage, const VkSharingMode sharingMode,
    const VkImageLayout initialLayout,
    const VkPhysicalDeviceMemoryProperties *memProps,
    const std::vector<uint32_t> &queueFamilies, VkImage *image,
    VkDeviceMemory *imageMemory) {
  VkResult ret = CreateImage(device, width, height, imageType, format, usage,
                             sharingMode, queueFamilies, initialLayout, image);
  if (ret != VK_SUCCESS) {
  ...
  allocInfo.memoryTypeIndex =
      GetRequiredMemTypeIndex(memProps, memRequirements,
...
VkResult VkGPUHelper::CreateStorageBufferAndBindMem(
    const VkDevice device, const VkDeviceSize size,
    const std::vector<uint32_t> &queueFamilyIndices,
    const VkPhysicalDeviceMemoryProperties *memProps, VkBuffer *storageBuffer,
    VkDeviceMemory *storageBufferMemory) {
  return CreateStorageBufferAndBindMem(device, size, queueFamilyIndices,
                                       memProps,
                                       VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                           VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
...
VkResult VkGPUHelper::CreateUniformBufferAndBindMem(
    const VkDevice device, const VkDeviceSize size,
    const std::vector<uint32_t> &queueFamilyIndices,
    const VkPhysicalDeviceMemoryProperties *memProps, VkBuffer *storageBuffer,
    VkDeviceMemory *storageBufferMemory) {
  VkResult ret = CreateBuffer(device, size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
...
VkResult VkGPUHelper::CreateVertexBufferAndBindMem(
    const VkDevice device, const VkDeviceSize size,
    const std::vector<uint32_t> &queueFamilyIndices,
    const VkPhysicalDeviceMemoryProperties *memProps, VkBuffer *storageBuffer,
    VkDeviceMemory *storageBufferMemory) {
  VkResult ret = CreateBuffer(device, size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                              VK_SHARING_MODE_EXCLUSIVE, queueFamilyIndices,
                              storageBuffer);
  if (ret != VK_SUCCESS) {
    Logger() << "vkCreateBuffer failed, err=" << string_VkResult(ret)
...
VkResult VkGPUHelper::CreateIndexBufferAndBindMem(
    const VkDevice device, const VkDeviceSize size,
    const std::vector<uint32_t> &queueFamilyIndices,
    const VkPhysicalDeviceMemoryProperties *memProps, VkBuffer *storageBuffer,
    VkDeviceMemory *storageBufferMemory) {
  VkResult ret = CreateBuffer(device, size, VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                              VK_SHARING_MODE_EXCLUSIVE, queueFamilyIndices,
                              storageBuffer);
  if (ret != VK_SUCCESS) {
    Logger() << "vkCreateBuffer failed, err=" << string_VkResult(ret)
