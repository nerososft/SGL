...
ComputePipelineNode::ComputePipelineNode(
    ...
    const std::string &shaderPath, const uint32_t pushConstantSize,
    const std::vector<VkDescriptorSetLayoutBinding>
        &descriptorSetLayoutBindings,
    const uint32_t workGroupCountX, const uint32_t workGroupCountY,
...
void ComputePipelineNode::AddComputeElement(
    const ComputeElement &computeElement) {
  this->computeElements.push_back(computeElement);
  const std::shared_ptr<VkGPUDescriptorSet> descriptorSet =
      CreateDescriptorSet(computeElement);
...
VkResult ComputePipelineNode::CreateComputeGraphNode() {
  ...
  computePipeline = std::make_shared<VkGPUComputePipeline>(
      shaderPath, descriptorSetLayoutBindings, pushConstantRanges);
  const VkResult ret = computePipeline->CreateComputePipeline(
  ...
  for (size_t i = 0; i < computeElements.size(); ++i) {
    ...
    for (const auto &buffer : computeElements[i].buffers) {
      if (buffer.type == PIPELINE_NODE_BUFFER_STORAGE_READ) {
        readBufferMemoryBarriers.push_back(
            VkGPUHelper::BuildBufferMemoryBarrier(
                VK_ACCESS_MEMORY_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT,
                buffer.buf.buffer, buffer.buf.bufferSize));
      }
    }

    PFN_vkCmdPipelineBarrier2KHR vkCmdPipelineBarrier2Fn =
        vkCmdPipelineBarrier2Fn = VkGPUHelper::GetVkCmdPipelineBarrier2Fn(
            this->gpuCtx->GetCurrentDevice());
    ...
    VkGPUHelper::GPUCmdPipelineBufferMemBarrier(
        ...
        VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, readBufferMemoryBarriers);

    VkGPUHelper::GPUCmdDispatch(commandBuffer, this->workGroupCountX,
                                workGroupCountY, workGroupCountZ);
    std::vector<VkBufferMemoryBarrier> writeBufferMemoryBarriers;
    for (const auto &buffer : computeElements[i].buffers) {
      if (buffer.type == PIPELINE_NODE_BUFFER_STORAGE_WRITE) {
        writeBufferMemoryBarriers.push_back(
            VkGPUHelper::BuildBufferMemoryBarrier(
                VK_ACCESS_MEMORY_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT,
                buffer.buf.buffer, buffer.buf.bufferSize));
      }
    ...
    VkGPUHelper::GPUCmdPipelineBufferMemBarrier(
        commandBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
        VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, writeBufferMemoryBarriers);
    std::vector<VkMemoryBarrier> memoryBarriers;
    memoryBarriers.push_back(VkGPUHelper::BuildMemoryBarrier(
        VK_ACCESS_MEMORY_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT));
    VkGPUHelper::GPUCmdPipelineMemBarrier(
        commandBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
        VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, memoryBarriers);
