...
std::shared_ptr<IComputeGraphNode> TransposeOperator::CreateComputeGraphNode() {
  ...
  const auto tNode = std::make_shared<ComputePipelineNode>(
      this->gpuCtx, "Transpose", SHADER(transpose.comp.glsl.spv),
      sizeof(TransposeOperatorParams), descriptorSetLayoutBindings,
      (this->params.height + 31) / 32, (this->params.width + 31) / 32, 1);
...
TransposeOperator::~TransposeOperator() { this->Destroy(); }

void TransposeOperator::Destroy() { UnaryOperator::Destroy(); }
