...
bool GaussianSplatting3DRenderer::InitializeGPUPipeline(
  ...
  bezierNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "3DGS", SHADER(3dgs.comp.glsl.spv),
      sizeof(GaussianSplatting3DParams), descriptorSetLayoutBindings,
      (params.numPoints + 255) / 256, 1, 1);
  ...
  result = inputBuffer->AllocateAndBind(GPU_BUFFER_TYPE_STORAGE_SHARED,
                                        maxPoints * sizeof(GaussianPoint));
  if (result != VK_SUCCESS) {
  ...
  ppBuffers.push_back({.type = PIPELINE_NODE_BUFFER_STORAGE_WRITE,
                       .buf = {.bufferSize = pixelMapBuffer->GetBufferSize(),
                               .buffer = pixelMapBuffer->GetBuffer()}});
  ppBuffers.push_back({.type = PIPELINE_NODE_BUFFER_STORAGE_WRITE,
                       .buf = {.bufferSize = depthMapBuffer->GetBufferSize(),
...
void GaussianSplatting3DRenderer::Render(
