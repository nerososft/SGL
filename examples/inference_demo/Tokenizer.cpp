...
bool Tokenizer::LoadVocabularyFromJsonObject(
    nlohmann::json::const_reference array) {
  if (!array.is_object()) {
    Logger() << "Not an object" << std::endl;
    return false;
  }
  Logger(Logger::DEBUG) << "Loading vocabulary from json object, size: "
                        << array.size() << std::endl;
...
bool Tokenizer::LoadVocabularyFromJsonFile(
    nlohmann::json::const_reference str) {
  // TODO:
  return false;
}
...
bool Tokenizer::LoadMergesFromJsonArray(nlohmann::json::const_reference array) {
  ...
  Logger(Logger::DEBUG) << "Loading merge from json array, size: "
                        << array.size() << std::endl;
...
bool Tokenizer::LoadFromFile(const std::string &filePath) {
  ...
  if (data["model"].is_null() || !data["model"].is_object()) {
    Logger() << "No model found, not a valid tokenizer file " << filePath
             << std::endl;
    return false;
  }
