...
aiMatrix4x4 ModelLoader::GetMeshTransform(const aiScene *scene,
  ...
  for (int meshIndex = 0; meshIndex < scene->mNumMeshes; meshIndex++) {
    ...
    if (scene->HasMaterials()) {
      ...
      aiColor3D ambientColor, diffuseColor, specularColor, emissiveColor,
          transparentColor, reflectiveColor;
      float shininess;
      aiString materialName;
      material->Get(AI_MATKEY_NAME, materialName);
      Logger() << "Loading material '" << materialName.C_Str() << "'"
               << std::endl;
      ...
      for (uint32_t textureType = aiTextureType_NONE;
           textureType < aiTextureType_UNKNOWN; textureType++) {
        uint32_t normalTextureCount =
            material->GetTextureCount(static_cast<aiTextureType>(textureType));
        Logger() << "Texture "
                 << aiTextureTypeToString(
                        static_cast<aiTextureType>(textureType))
                 << " count: " << normalTextureCount << std::endl;
        for (uint32_t i = 0; i < normalTextureCount; i++) {
          aiString texturePath;
          material->GetTexture(static_cast<aiTextureType>(textureType), i,
                               &texturePath);
          Logger() << "Loading "
                   << aiTextureTypeToString(
                          static_cast<aiTextureType>(textureType))
