...
void GLFWWindowImpl::CreateWindow(uint32_t posX, uint32_t posY, const uint32_t width, const uint32_t height,
                                  const char *title) {
    if (!glfwInitialized) {
    ...
    Logger() << "Creating window '" << title << "', width=" << width << ", height=" << height << std::endl;
...
void GLFWWindowImpl::PollEvent() {
    glfwPollEvents();
}

void GLFWWindowImpl::Render() {
...
std::vector<const char *> GLFWWindowImpl::GetRendererRequiredExtensions() {
    ...
    Logger() << "GLFW window extensions(count:" << glfwExtensionCount << "): " << std::endl;
    std::vector<const char *> requiredExtensions;
    requiredExtensions.reserve(glfwExtensionCount);
    for (int i = 0; i < glfwExtensionCount; i++) {
        Logger() << "GLFW window extension(" << i << "): " << extensions[i] << std::endl;
...
Surface GLFWWindowImpl::GetSurface(const SurfaceGetParams params) {
    ...
    glfwCreateWindowSurface(params.params.vkInstance, this->window, nullptr, &vkSurface);
