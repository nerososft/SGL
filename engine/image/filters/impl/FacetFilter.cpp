...
FacetFilter::Apply(const std::shared_ptr<VkGPUContext> &gpuCtx,
  ...
  this->facetParams.imageSize.bytesPerLine =
      this->facetParams.imageSize.width * 4;
  params.paramsSize = sizeof(FacetFilterParams);
  params.paramsData = &this->facetParams;
  params.shaderPath = SHADER(facet.comp.glsl.spv);

  return BasicFilter::Apply(gpuCtx, "Facet", inputImageInfo[0].bufferSize,
...
void FacetFilter::Destroy() { BasicFilter::Destroy(); }
