...
SpherizeFilter::Apply(const std::shared_ptr<VkGPUContext> &gpuCtx,
  ...
  this->spherizeFilterParams.imageSize.bytesPerLine =
      this->spherizeFilterParams.imageSize.width * 4;
  ...
  return BasicFilter::Apply(gpuCtx, "Spherize", inputImageInfo[0].bufferSize,
...
void SpherizeFilter::Destroy() { BasicFilter::Destroy(); }
