...
VkResult
CloudsFilter::Apply(const std::shared_ptr<VkGPUContext> &gpuCtx,
                    const std::vector<FilterImageInfo> &inputImageInfo,
                    const std::vector<FilterImageInfo> &outputImageInfo) {
  BasicFilterParams params;
  this->cloudsFilterParams.imageSize.width = inputImageInfo[0].width;
  this->cloudsFilterParams.imageSize.height = inputImageInfo[0].height;
  this->cloudsFilterParams.imageSize.channels = 4;
  this->cloudsFilterParams.imageSize.bytesPerLine =
      this->cloudsFilterParams.imageSize.width * 4;
  ...
  pushConstantInfo.size = sizeof(CloudsFilterParams);
  pushConstantInfo.data = &this->cloudsFilterParams;

  lookupBuffer = std::make_shared<VkGPUBuffer>(gpuCtx);
  lookupBuffer->AllocateAndBind(GPU_BUFFER_TYPE_UNIFORM, size * sizeof(int));
  lookupBuffer->UploadData(lookup, size * sizeof(int));
  ...
  pipelineNodeKInput.buf.buffer = lookupBuffer->GetBuffer();
  ...
  const auto kCalculateNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "clouds", SHADER(clouds.comp.glsl.spv), pushConstantInfo.size,
