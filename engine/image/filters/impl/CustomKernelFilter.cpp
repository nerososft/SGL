...
VkResult
CustomKernelFilter::Apply(const std::shared_ptr<VkGPUContext> &gpuCtx,
  ...
  this->kFilterParams.imageSize.bytesPerLine =
      this->kFilterParams.imageSize.width * 4;
  ...
  const auto kCalculateNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "kCalculate", SHADER(custom_kernel.comp.glsl.spv),
