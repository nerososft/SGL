...
VkResult
CrystallizeFilter::Apply(const std::shared_ptr<VkGPUContext> &gpuCtx,
  ...
  this->crystallizeFilterParams.imageSize.bytesPerLine =
      this->crystallizeFilterParams.imageSize.width * 4;
  ...
  const auto kCalculateNode = std::make_shared<ComputePipelineNode>(
      gpuCtx, "crystallize", SHADER(crystallize.comp.glsl.spv),
