...
RoPEMulOperator::RoPEMulOperator(const std::shared_ptr<VkGPUContext> &gpuCtx,
    ...
    : BinaryOperator(gpuCtx, Q, K, output) {}
...
std::shared_ptr<IComputeGraphNode> RoPEMulOperator::CreateComputeGraphNode() {
  ...
  auto ropeMulNode = std::make_shared<ComputePipelineNode>(
      this->gpuCtx, "RoPEMul", SHADER(rope_mul.comp.glsl.spv), 0,
  ...
  const PushConstantInfo pushConstantInfo{.size = sizeof(RoPEMulOperatorParams),
                                          .data = &this->params};
...
void RoPEMulOperator::Destroy() { BinaryOperator::Destroy(); }
