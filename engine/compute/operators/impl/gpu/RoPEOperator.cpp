...
std::shared_ptr<IComputeGraphNode> RoPEOperator::CreateComputeGraphNode() {
  ...
  auto ropeNode = std::make_shared<ComputePipelineNode>(
      this->gpuCtx, "RoPE", SHADER(rope.comp.glsl.spv), 0,
  ...
  const PushConstantInfo pushConstantInfo{.size = sizeof(RoPEOperatorParams),
                                          .data = &this->params};
...
void RoPEOperator::Destroy() { UnaryOperator::Destroy(); }
