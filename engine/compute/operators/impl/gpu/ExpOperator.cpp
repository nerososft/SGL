...
ExpOperator::ExpOperator(const std::shared_ptr<VkGPUContext> &gpuCtx,
...
std::shared_ptr<IComputeGraphNode> ExpOperator::CreateComputeGraphNode() {
  ...
  auto expNode = std::make_shared<ComputePipelineNode>(
      this->gpuCtx, "Exp", SHADER(exp.comp.glsl.spv), 0,
      descriptorSetLayoutBindings, (nums + 255) / 256, 1, 1);
  const VkResult ret = expNode->CreateComputeGraphNode();
  if (ret != VK_SUCCESS) {
  ...
  expNode->AddComputeElement(computeElem);

  return expNode;
}

void ExpOperator::Destroy() { UnaryOperator::Destroy(); }
