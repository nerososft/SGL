...
std::shared_ptr<IComputeGraphNode> SoftmaxOperator::CreateComputeGraphNode() {
  ...
  auto softmaxNode = std::make_shared<ComputePipelineNode>(
      this->gpuCtx, "Softmax", SHADER(softmax.comp.glsl.spv),
      sizeof(SoftmaxOperatorParams), descriptorSetLayoutBindings,
      (nums + 255) / 256, 1, 1);
  ...
  const PushConstantInfo pushConstantInfo{.size = sizeof(SoftmaxOperatorParams),
                                          .data = &this->params};
  const ComputeElement computeElem{.pushConstantInfo = pushConstantInfo,
                                   ...
                                   .preCompute = [this] {
                                     ...
                                     const float maxVal = this->max == nullptr
                                                              ? 0.0f
                                                              : *this->max;
...
void SoftmaxOperator::Destroy() { UnaryOperator::Destroy(); }
