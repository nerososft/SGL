...
std::shared_ptr<IComputeGraphNode> RMSNormOperator::CreateComputeGraphNode() {
  ...
  auto rmsNormNode = std::make_shared<ComputePipelineNode>(
      this->gpuCtx, "RSMNorm", SHADER(rms_norm.comp.glsl.spv), 0,
  ...
  const PushConstantInfo pushConstantInfo{.size = sizeof(RMSNormOperatorParams),
                                          .data = &this->params};
...
void RMSNormOperator::Destroy() { UnaryOperator::Destroy(); }
