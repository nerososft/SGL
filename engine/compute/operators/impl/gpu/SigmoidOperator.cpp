...
std::shared_ptr<IComputeGraphNode> SigmoidOperator::CreateComputeGraphNode() {
  ...
  descriptorSetLayoutBindings.push_back(
      VkGPUHelper::BuildDescriptorSetLayoutBinding(
          ...
          VK_SHADER_STAGE_COMPUTE_BIT));
  auto sigmoidNode = std::make_shared<ComputePipelineNode>(
      this->gpuCtx, "Sigmoid", SHADER(sigmoid.comp.glsl.spv), 0,
...
void SigmoidOperator::Destroy() { UnaryOperator::Destroy(); }
