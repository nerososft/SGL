...
AddOperator::AddOperator(const std::shared_ptr<VkGPUContext> &gpuCtx,
...
std::shared_ptr<IComputeGraphNode> AddOperator::CreateComputeGraphNode() {
  ...
  auto addNode = std::make_shared<ComputePipelineNode>(
      this->gpuCtx, "Add", SHADER(add.comp.glsl.spv), 0,
      descriptorSetLayoutBindings, (nums + 255) / 256, 1, 1);
  const VkResult ret = addNode->CreateComputeGraphNode();
  if (ret != VK_SUCCESS) {
    Logger() << "Error creating add node." << std::endl;
    return nullptr;
  }
  ...
  addNode->AddComputeElement(computeElem);
  return addNode;
}

AddOperator::~AddOperator() { this->Destroy(); }

void AddOperator::Destroy() { BinaryOperator::Destroy(); }
