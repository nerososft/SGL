...
ConcatOperator::ConcatOperator(const std::shared_ptr<VkGPUContext> &gpuCtx,
                               const std::vector<std::shared_ptr<VkGPUBuffer> > &inputBuffers,
                               const std::shared_ptr<VkGPUBuffer> &outBuffer): Operator(gpuCtx) {
...
std::shared_ptr<IComputeGraphNode> ConcatOperator::CreateComputeGraphNode() {
    ...
    for (size_t i = 0; i < this->params.nums; ++i) {
        descriptorSetLayoutBindings.push_back(
            VkGPUHelper::BuildDescriptorSetLayoutBinding(i, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                                                         VK_SHADER_STAGE_COMPUTE_BIT));
    }
    descriptorSetLayoutBindings.push_back(
        VkGPUHelper::BuildDescriptorSetLayoutBinding(this->params.nums, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                                                     VK_SHADER_STAGE_COMPUTE_BIT));
    ...
    const auto concatNode = std::make_shared<ComputePipelineNode>(this->gpuCtx,
                                                                  "Concat",
                                                                  shaderSpv,
                                                                  0,
